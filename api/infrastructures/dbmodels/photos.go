...
func (r *photoR) GetPhotoExifs() PhotoExifSlice {
	if r == nil {
		return nil
	}
	return r.PhotoExifs
...
// PhotoExifs retrieves all the photo_exif's PhotoExifs with an executor.
func (o *Photo) PhotoExifs(mods ...qm.QueryMod) photoExifQuery {
	...
	queryMods = append(queryMods,
		qm.Where("`photo_exif`.`photo_id`=?", o.PhotoID),
	)

	return PhotoExifs(queryMods...)
}
...
func (photoL) LoadPhotoExifs(ctx context.Context, e boil.ContextExecutor, singular bool, maybePhoto interface{}, mods queries.Applicator) error {
	...
	query := NewQuery(
		qm.From(`photo_exif`),
		qm.WhereIn(`photo_exif.photo_id in ?`, argsSlice...),
	)
	if mods != nil {
		mods.Apply(query)
	}
	...
	if err != nil {
		return errors.Wrap(err, "failed to eager load photo_exif")
	}

	var resultSlice []*PhotoExif
	if err = queries.Bind(results, &resultSlice); err != nil {
		return errors.Wrap(err, "failed to bind eager loaded slice photo_exif")
	}

	if err = results.Close(); err != nil {
		return errors.Wrap(err, "failed to close results in eager load on photo_exif")
	}
	...
	if singular {
		object.R.PhotoExifs = resultSlice
	...
	for _, foreign := range resultSlice {
		for _, local := range slice {
			if local.PhotoID == foreign.PhotoID {
				local.R.PhotoExifs = append(local.R.PhotoExifs, foreign)
				if foreign.R == nil {
					foreign.R = &photoExifR{}
				}
...
func (o *Photo) AddPhotoExifs(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*PhotoExif) error {
	var err error
	for _, rel := range related {
		if insert {
			...
			values := []interface{}{o.PhotoID, rel.PhotoExifID}
	...
	if o.R == nil {
		...
		o.R.PhotoExifs = append(o.R.PhotoExifs, related...)
	}
