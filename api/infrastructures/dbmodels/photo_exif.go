...
var PhotoExifTableColumns = struct {
	...
	PhotoExifID: "photo_exif.photo_exif_id",
	PhotoID:     "photo_exif.photo_id",
	TagID:       "photo_exif.tag_id",
	TagName:     "photo_exif.tag_name",
	TagType:     "photo_exif.tag_type",
	ValueString: "photo_exif.value_string",
	SortOrder:   "photo_exif.sort_order",
...
var PhotoExifWhere = struct {
	...
	ValueString: whereHelperstring{field: "`photo_exif`.`value_string`"},
	SortOrder:   whereHelperint{field: "`photo_exif`.`sort_order`"},
...
func (r *photoExifR) GetPhoto() *Photo {
	if r == nil {
		return nil
	}
	return r.Photo
...
func (photoExifL) LoadPhoto(ctx context.Context, e boil.ContextExecutor, singular bool, maybePhotoExif interface{}, mods queries.Applicator) error {
	...
	if singular {
		var ok bool
		object, ok = maybePhotoExif.(*PhotoExif)
		if !ok {
			object = new(PhotoExif)
			ok = queries.SetFromEmbeddedStruct(&object, &maybePhotoExif)
			if !ok {
				return errors.New(fmt.Sprintf("failed to set %T from embedded struct %T", object, maybePhotoExif))
			}
		}
	} else {
		s, ok := maybePhotoExif.(*[]*PhotoExif)
		if ok {
			slice = *s
		} else {
			ok = queries.SetFromEmbeddedStruct(&slice, maybePhotoExif)
			if !ok {
				return errors.New(fmt.Sprintf("failed to set %T from embedded struct %T", slice, maybePhotoExif))
			}
		}
	}
	...
	if singular {
		...
		foreign.R.PhotoExifs = append(foreign.R.PhotoExifs, object)
		return nil
	}
	...
	for _, local := range slice {
		for _, foreign := range resultSlice {
			if local.PhotoID == foreign.PhotoID {
				...
				foreign.R.PhotoExifs = append(foreign.R.PhotoExifs, local)
				break
			}
		}
	}
...
// Adds o to related.R.PhotoExifs.
func (o *PhotoExif) SetPhoto(ctx context.Context, exec boil.ContextExecutor, insert bool, related *Photo) error {
	...
	values := []interface{}{related.PhotoID, o.PhotoExifID}
	...
	if related.R == nil {
		related.R = &photoR{
			PhotoExifs: PhotoExifSlice{o},
		}
	} else {
		related.R.PhotoExifs = append(related.R.PhotoExifs, o)
	}
...
func PhotoExifs(mods ...qm.QueryMod) photoExifQuery {
	...
	if len(queries.GetSelect(q)) == 0 {
		queries.SetSelect(q, []string{"`photo_exif`.*"})
	}

	return photoExifQuery{q}
}
...
func FindPhotoExif(ctx context.Context, exec boil.ContextExecutor, photoExifID string, selectCols ...string) (*PhotoExif, error) {
	...
	query := fmt.Sprintf(
		"select %s from `photo_exif` where `photo_exif_id`=?", sel,
	)

	q := queries.Raw(query, photoExifID)
...
func (o *PhotoExif) Upsert(ctx context.Context, exec boil.ContextExecutor, updateColumns, insertColumns boil.Columns) error {
	...
	if !cached {
		...
		ret := strmangle.SetComplement(photoExifAllColumns, strmangle.SetIntersect(insert, update))
...
func (o *PhotoExif) Delete(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	sql := "DELETE FROM `photo_exif` WHERE `photo_exif_id`=?"
...
func (o *PhotoExif) Reload(ctx context.Context, exec boil.ContextExecutor) error {
	ret, err := FindPhotoExif(ctx, exec, o.PhotoExifID)
	if err != nil {
		return err
	}
...
func PhotoExifExists(ctx context.Context, exec boil.ContextExecutor, photoExifID string) (bool, error) {
	...
	if boil.IsDebug(ctx) {
		writer := boil.DebugWriterFrom(ctx)
		fmt.Fprintln(writer, sql)
		fmt.Fprintln(writer, photoExifID)
	}
	row := exec.QueryRowContext(ctx, sql, photoExifID)
...
// Exists checks if the PhotoExif row exists.
func (o *PhotoExif) Exists(ctx context.Context, exec boil.ContextExecutor) (bool, error) {
	return PhotoExifExists(ctx, exec, o.PhotoExifID)
}
